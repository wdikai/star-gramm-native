...
const getPosts = (length, count) =>
    Array.from({ length }).map((_, i) => ({
        id: i + 1 + count,
        creator: getUser(i + 1 + count),
        description: !(i % 3)
            ? ''
            : `Test ${i +
...
export default class PostFeed {
    ...
    @action
    async fetchPosts(isRefresh) {
        const previous = isRefresh ? [] : this.posts;
        this.isLoading = true;
        await timeout(1000);
        this.posts = previous.concat(getPosts(20, previous.length));
